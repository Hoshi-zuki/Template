...
struct SegTree {
    ...
    void set(int jobl, int jobr, type val, int ind = 1) { //为jobl~jobr 设置为val
        auto &rt = Node[ind];
        ...
        down(ind);
        set(jobl, jobr, val, ind << 1);
        set(jobl, jobr, val, ind << 1 | 1);
        up(ind);
    }
